...
import {
  ...
  LEGACY_CURSORRULES_MARKER_BEGIN,
  LEGACY_CURSORRULES_MARKER_END,
} from "./constants.js";
import {
  appendFileSafe,
  readFileSafe,
...
const MARKER_PAIRS = [
  ...
  [LEGACY_CURSORRULES_MARKER_BEGIN, LEGACY_CURSORRULES_MARKER_END],
];
 ...
 */
function locateSeatbeltBlock(content) {
...
function extractSeatbeltBlock(content) {
  const located = locateSeatbeltBlock(content);
  if (!located) {
    return null;
  }
...
function replaceSeatbeltBlock(content) {
  const located = locateSeatbeltBlock(content);
  if (!located) {
    return null;
  }
...
export async function injectCursorRules(cwd) {
  ...
  try {
    ...
    const currentBlock = extractSeatbeltBlock(existing);
    ...
    if (currentBlock) {
      ...
      const replaced = replaceSeatbeltBlock(existing);
      if (replaced === null) {
        throw new Error(`Failed to upgrade seatbelt block in ${rulesPath}`);
      }
