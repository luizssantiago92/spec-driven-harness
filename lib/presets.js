...
import { mergeSeatbeltConfigs, parseSeatbeltConfig, resolveSeatbeltConfig } from "./config.js";
...
/**
 * @param {ReturnType<typeof parseSeatbeltConfig>} document
 * @param {Set<string>} [seen]
 * @returns {Promise<ReturnType<typeof parseSeatbeltConfig>>}
 */
export async function resolveConfigDocument(document, seen = new Set()) {
  return resolveSeatbeltConfig(document, loadPreset, seen);
}

/**
 * @param {string} [cwd]
 * @returns {Promise<ReturnType<typeof parseSeatbeltConfig> | null>}
 */
export async function loadResolvedConfig(cwd = process.cwd()) {
  ...
  try {
    ...
    const document = parseSeatbeltConfig(text);
...
export function configFromPreset(preset) {
  if (preset === "default") {
    return `# Project seatbelt config (preset: default)
...
`;
  ...
  return `# Project seatbelt config (extends preset: ${preset})
extends: ${preset}
 ...
 *
 * @param {ReturnType<typeof parseSeatbeltConfig> | null | undefined} config
