...
export const CLI_NAME = "spec-seatbelt";
export const DISPLAY_NAME = "Spec Seatbelt";
export const NPX = (command) => `npx ${PACKAGE_NAME}${command ? ` ${command}` : ""}`;

const REPO_RAW_BASE =
  "https://raw.githubusercontent.com/luizssantiago92/spec-seatbelt";
...
/** @deprecated Pre-v2.2 installs; resolved at runtime when seatbelt path is missing */
...
export const CURSORRULES_MARKER_BEGIN = "<!-- SPEC-SEATBELT:BEGIN -->";

export const CURSORRULES_MARKER_END = "<!-- SPEC-SEATBELT:END -->";
...
# Execution Contract (Spec Seatbelt)
...
- Scripts in \`.specs/seatbelt/scripts/\` — the **agent** runs them at phase boundaries (see hub).
- Humans: \`install\` once; optional \`feature-init\`, \`project-init\`, \`doctor\`. Full CLI: \`npx @luizsantiago/spec-seatbelt --help\`.
...
function assertAllowedAssetUrl(raw) {
  ...
  try {
    ...
    throw new Error(`Invalid seatbelt asset URL: ${raw}`);
  }
  ...
  throw new Error(
    `Refusing to download seatbelt assets over ${url.protocol}//${url.hostname} ` +
      "— only HTTPS sources are allowed. Seatbelt assets include executable gate scripts.",
  );
}
...
export function resolveAssetOverride() {
  const override =
    process.env.SPEC_SEATBELT_REPO_URL ?? process.env.HARNESS_REPO_URL;
