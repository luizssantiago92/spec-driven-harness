...
/**
 * Absolute path of a harness asset shipped inside the npm package.
 *
...
export function packagedAssetPath(remotePath) {
  return path.join(PACKAGE_ROOT, remotePath);
}

/**
 * Default install copies from the package. A remote fetch happens only when
 * `HARNESS_REPO_URL` or `options.repoUrl` is set (forks and the test suite).
 *
...
export function resolveInstallSource(repoUrl) {
  ...
  if (override) {
    return { mode: "remote", repoUrl: override };
  }
  return { mode: "package" };
}
...
export async function copyPackagedAsset(remotePath, destPath) {
  ...
  try {
    ...
    if (err.code === "ENOENT") {
      throw new Error(
        `Packaged harness asset missing: ${remotePath}. ` +
          "Reinstall @luizsantiago/agentic-harness.",
      );
    }
