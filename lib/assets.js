...
/**
 * Absolute path of a seatbelt asset shipped inside the npm package.
 *
 ...
 * `SPEC_SEATBELT_REPO_URL` (or legacy `HARNESS_REPO_URL`) or `options.repoUrl` is set (forks and the test suite).
 *
...
export async function copyPackagedAsset(remotePath, destPath) {
  ...
  try {
    ...
    if (err.code === "ENOENT") {
      throw new Error(
        `Packaged seatbelt asset missing: ${remotePath}. ` +
