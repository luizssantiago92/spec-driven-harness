...
import { HARNESS_SCRIPTS_DIR, SKILL_DIRS } from "./constants.js";
...
export async function runDoctorChecks(cwd) {
  ...
  checks.push({
    ...
    weight: 12,
    pass: hubInstalled,
    suggest: "npx @luizsantiago/agentic-harness install",
  });

  const gatesDir = path.join(cwd, HARNESS_SCRIPTS_DIR);
  ...
  checks.push({
    id: "gate-scripts",
    label: "Python gate scripts under .specs/harness/scripts/",
    weight: 15,
    pass: gatesPresent,
    suggest: "npx @luizsantiago/agentic-harness install",
  });
  ...
  checks.push({
    ...
    suggest: "npx @luizsantiago/agentic-harness init-config --preset default",
  });
  ...
  checks.push({
    ...
    pass: hasProjectMd,
    optional: true,
    suggest: "npx @luizsantiago/agentic-harness project-init",
  });
  ...
  if (gatesPresent && pythonOk) {
    try {
      const script = path.join(cwd, HARNESS_SCRIPTS_DIR, "check_commit.py");
      await execFileAsync("python3", [
        script,
        "--message",
        "chore(harness): doctor smoke test",
      ], { cwd });
      gateSmoke = true;
    } catch {
      gateSmoke = false;
  ...
  checks.push({
    ...
    suggest: gatesPresent
      ? "Fix Python gate install or run: python3 .specs/harness/scripts/check_commit.py --message \"test: smoke\""
 ...
 * @returns {Promise<{ score: number, checks: DoctorCheck[], suggestions: DoctorCheck[] }>}
 */
export async function doctor(cwd, options = {}) {
  ...
  if (options.json) {
    console.log(JSON.stringify({ score, checks, suggestions }, null, 2));
    return { score, checks, suggestions };
  }

  console.log(`Harness Ready: ${score}/100\n`);
