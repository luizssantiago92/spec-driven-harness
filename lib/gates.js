...
import { HARNESS_SCRIPTS_DIR } from "./constants.js";
...
export async function runGate(gate, args, options = {}) {
  ...
  try {
    ...
    throw new Error(
      `Gate script not found at ${path.join(HARNESS_SCRIPTS_DIR, scriptName)}. ` +
        "Run `npx @luizsantiago/agentic-harness install` in this project first.",
    );
  }
