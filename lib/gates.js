...
const HARNESS_SCRIPTS = { ...GATE_SCRIPTS, ...AUX_SCRIPTS };
...
/**
 * Run a harness Python script installed under .specs/harness/scripts/.
 *
 ...
 */
export async function runHarnessScript(command, args, options = {}) {
  const scriptName = HARNESS_SCRIPTS[command];
  ...
  for (const required of [scriptName, "_common.py"]) {
    try {
      ...
      throw new Error(
        `Harness script not found at ${path.join(HARNESS_SCRIPTS_DIR, required)}. ` +
  ...
  if (!python) {
    throw new Error(
      "Python 3 not found. Install Python 3.10+ to run harness scripts, " +
...
export async function runGate(gate, args, options = {}) {
  ...
  return runHarnessScript(gate, args, options);
}
