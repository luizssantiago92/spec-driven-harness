...
import { initSeatbeltMemory } from "./memory.js";
...
export async function install(options = {}) {
  ...
  if (!options.repoUrl && override) {
    log(
      `⚠️  SPEC_SEATBELT_REPO_URL is set — installing skills and executable gate ` +
  ...
  log(`✅ ${SCRIPT_ASSETS.length} scripts → ${SEATBELT_SCRIPTS_DIR}`);
  ...
  const { stateCreated, lessonsCreated } = await initSeatbeltMemory(cwd);
