...
export const SKILLS_MAP_START = "<!-- seatbelt-managed:skills-map:start -->";
export const SKILLS_MAP_END = "<!-- seatbelt-managed:skills-map:end -->";

/** Markers around the gates table — refreshed on every install. */
export const GATES_MAP_START = "<!-- seatbelt-managed:gates-map:start -->";
export const GATES_MAP_END = "<!-- seatbelt-managed:gates-map:end -->";

const LEGACY_SKILLS_MAP_START = "<!-- harness-managed:skills-map:start -->";
const LEGACY_SKILLS_MAP_END = "<!-- harness-managed:skills-map:end -->";
const LEGACY_GATES_MAP_START = "<!-- harness-managed:gates-map:start -->";
const LEGACY_GATES_MAP_END = "<!-- harness-managed:gates-map:end -->";
...
export function extractSkillsMapBlock(content) {
  return extractFirstManagedBlock(content, [
    ...
    [LEGACY_SKILLS_MAP_START, LEGACY_SKILLS_MAP_END],
  ]);
}
...
export function extractGatesMapBlock(content) {
  return extractFirstManagedBlock(content, [
    ...
    [LEGACY_GATES_MAP_START, LEGACY_GATES_MAP_END],
  ]);
}
...
export function mergeBaselineRule(existing, shipped) {
  ...
  if (shippedSkills) {
    const skillsSection = /^# (?:Harness|Seatbelt) Skills\b[\s\S]*?(?=^# |\Z)/m;
    if (
      !extractSkillsMapBlock(merged) &&
      skillsSection.test(merged) &&
      shipped.match(/^# (?:Harness|Seatbelt) Skills\b[\s\S]*?(?=^# |\Z)/m)
    ) {
      const shippedSection = shipped.match(/^# (?:Harness|Seatbelt) Skills\b[\s\S]*?(?=^# |\Z)/m);
      if (shippedSection) {
      ...
      merged = mergeManagedBlock(
        ...
        SKILLS_MAP_END,
        [[LEGACY_SKILLS_MAP_START, LEGACY_SKILLS_MAP_END]],
        extractSkillsMapBlock(merged) ? "" : "# Seatbelt Skills",
      );
    }
  }
