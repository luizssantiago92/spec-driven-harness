...
| `validate_state` | Missing `validation.md`, verdict other than exact `PASS`/`PASSED` (`PASS WITH GAPS` fails), no **test** `file:line` evidence, a spec requirement without same-line test evidence, open task, Medium+ feature without a discrimination-sensor result |
